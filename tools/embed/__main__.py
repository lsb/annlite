...
def _read_texts(path: Path, start: int, end: int) -> list[str]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        for i, line in enumerate(f):
            if i >= end:
                break
            if i >= start:
                out.append(json.loads(line)["text"] if jsonl else line.rstrip("\n"))
    return out


def _worker(args):
    inp, outp, start, end, rows, batch, threads, model, vocab = args
    enc = DenseEncoder(model, vocab, threads=threads)
    mm = store.open_memmap(outp, rows, EMBED_DIM, mode="r+")
    texts = _read_texts(Path(inp), start, end)
    pos = start
    t0 = time.time()
...
def main() -> int:
    ...
    store.write_manifest(
        ...
        pooling="masked-mean", normalized="l2",
