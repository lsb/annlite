...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        ...
        succ(&rank_exact, 1), succ(&rank_exact, 10), mrr(&rank_exact),
    ...
    // Centroid only, for the bottom of the range.
    {
        ...
        let ranks: Vec<usize> = (0..queries.len())
            ...
            .collect();
        println!("{:>6} {:>10} {:>10} {:>10} {:>9.3} {:>9.3} {:>9.3}",
                 "none", 4, per_doc, format!("{:.0}x", (dim * 4) as f64 / 4.0),
                 succ(&ranks, 1), succ(&ranks, 10), mrr(&ranks));
    }
    ...
    for bits in [1u8, 2, 4] {
        ...
        let per_doc = (store.bytes() + total * 4 + idx.centroids.len() * 4 + store.codec.table_bytes())
            / docs.len();
